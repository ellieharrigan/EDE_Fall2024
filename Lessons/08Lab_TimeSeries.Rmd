...
#create sequence function from first observation to last observation (use join function)
...
BR_complete.rain.ts <- ts(BR_complete$Rainfall, 
                          start = c(1901,1), 
                          frequency = 12)

BR_complete.temp.ts <- ts(BR_complete$Temperature_C, 
                          start = c(1901,1), 
                          frequency = 12)
...
BR_rain_decomposed <- stl(BR_complete.rain.ts,
                          s.window = "periodic")
...
BR_temp_decomposed <- stl(BR_complete.temp.ts,
                          s.window = "period")
...
BR_rain_components <- as.data.frame(BR_rain_decomposed$time.series)

BR_temp_components <- as.data.frame(BR_temp_decomposed$time.series)
...
ggplot(BR_rain_components) +
  ...
  geom_line(aes(y= trend), color = "red") +
    geom_hline(y= seasonal), color = "blue")
