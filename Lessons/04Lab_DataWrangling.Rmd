...
glimpse(NTL.phys.data$sampledate) #similar to str command, output is a little different
...
```{r Exercise 1 & 2}
# Exercise 1: 
# NTL.phys.data for the year 1999
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999) or
EX1 <- filter(NTL.phys.data, sampledate == 1999)
...
> Answer: We don't filter using row numbers because we don't have control over the row numbers.
...
Ex6 <- pivot_wider(Ex5, names_from = temperature_C, values_from = depth)  
