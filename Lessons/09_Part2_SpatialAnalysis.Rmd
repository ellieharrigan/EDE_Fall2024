...
huc8_UpperNeuse <- huc8_sf %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
mapview(huc8_sf, col.regions = 'grey') + mapview(huc8_UpperNeuse, 
                                                 color = 'blue',
                                           col.regions = 'yellow',
                                           lwd = 2)
...
wind_data <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  ...
  group_by(HUC_8) %>% 
  summarise(wind = sum(AvgWindEnergy, na.rm = TRUE))
...
neuse_intersect <- counties_sf_utm %>%  
  st_filter(neuse_sf, .predicate = st_intersects) #can we go over predicate commmand
...
huc_8_intersect <- counties_sf_utm %>% 
  st_filter(triCo, .predicate = st_intersects)

mapview(huc_8_intersect)

#Intersect the HUC_8s
HUC_8_counties <- triCo %>% st_intersection(huc_8_intersect)

mapview(HUC_8_counties)
```
