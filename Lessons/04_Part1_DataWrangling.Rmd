...
`==` use this operator when want to compare your value to a specific value
`!=` if you want to check if variables are different, or exclude variable from dataset
`<`check for values that are less than a specific number
`<=`check for values that are less than or equal to specific number
`>`check for values that are greater than a specific value
`>=`check fo values that are greater than or equal to speific value
`&`if you have multiple values to check you aggregate them, this is not one or the other but a combination of both
`|`value for "OR"
'%in%' will select the names from a column that you want to extract from dataset, you run this command with the name of the column and then create a vector with specified variables from that column you want to extract
...
# these examples show how to filter CHARACTER variables
summary(NTL.phys.data$lakename)
#sort(summary(NTL.phys.data$lakename), decreasing = TRUE) #shows lake names that occur in order of highest to lowest
...
# These examples show how to filter for a NUMERIC or INTEGER variable
...
#Use == logical commands

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.TuesdayLake <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 >= 1990 & year4 <= 1999)
#or
NTL.phys.data.TuesdayLake1 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Because variables that you want to select may not be in sequential order by row number, which could make your code messy and room for more errors if you are skimming through the dataset to select variables by the row numbers. 
...
NTL.phys.data.temperature.ascending <- arrange(NTL.phys.data, temperature_C)
# Which dates, lakes, and depths have the highest temperatures?
#1998-07-16, East Long Lake, .50 depth - temperature is 34.1 degrees celcius 
