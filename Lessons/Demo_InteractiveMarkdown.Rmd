...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    ...
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) + #alpha is transparency, good to apply when have overlapping features
  scale_x_date(
    date_breaks = "1 years",
    date_labels = '%Y') +
  scale_shape_manual(values = c(17, 15)) + #this assigns certain shapes to your plot, google ggplot shapes or markers to get cheats sheet
  labs(
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = .data[[the_parameter]], #this is what you have to use to assign a variable to what the code thinks is a column
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) + #alpha is transparency, good to apply when have overlapping features
  scale_x_date(
