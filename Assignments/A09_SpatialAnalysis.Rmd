...
> ANSWER: 
> The EPSG code is 4269, this is a geographic coordinate system and the CRS uses angular coordinate units. The datum this CRS is associated with is NAD83. 
...
>ANSWER: 
> The new field that appears in the sf dataframe is 'geometry'. The fields that disappear are 'dec_lat_va' and 'dec_long_va'.  
...
gage_counties_plot <- ggplot() +
  geom_sf(data = counties_sf) +
  geom_sf(data = NWIS_gage_sf, color = "yellow", size = 2) +
  #scale_fill_gradient(low = "lightgrey", high = "purple") +
  labs(
    title = "NWIS Gage Locations in Nebraska",
       subtitle = 'Ellie Harrigan'
)

print(gage_counties_plot)
```
...
NWIS_site_flow <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_RAW.csv'),
                       stringsAsFactors = TRUE,
                       colClasses = c('gage_ht' = 'factor'))
...
```{r map gage sites}
mapview(counties_sf, color = "lightgray", alpha = 0.2, legend = FALSE) +
...
#21 Plot the data
mapview(counties_sf_utm, col.regions = 'lightgrey') + 
  mapview(NWIS_gage_sf_utm, col.regions = 'yellow')
```
...
lancaster_gage_sites <- st_transform(lancaster_county, st_crs(NWIS_gage_sf_utm))

lancaster_gage_sites <- st_intersection(NWIS_gage_sf_utm, lancaster_gage_sites)

  
#24 Spatially select gages within 15 k of the selected county
lancaster_buffer <- NWIS_gage_sf %>% 
  st_buffer(., dist=15000)

lancaster_county_transform <- st_transform(lancaster_county, 
                                           st_crs(NWIS_gage_sf_utm))

lancaster_county_buffer <- st_buffer(lancaster_county_transform,
                                           dist= 15000)

gages_15km <- st_intersection(NWIS_gage_sf_utm, lancaster_county_buffer)



#25 Plot
mapview(counties_sf_utm, col.regions = 'grey', alpha = .5) + 
mapview(lancaster_county, col.regions = 'green') +
  mapview(lancaster_gage_sites, col.regions = 'blue') +
  mapview(gages_15km, col.regions= 'red', color = 'red', alpha = -.5)
